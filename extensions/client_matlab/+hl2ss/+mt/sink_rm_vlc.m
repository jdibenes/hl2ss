...
methods
    function self = sink_rm_vlc(host, port, chunk, mode, divisor, profile, level, bitrate, options, buffer_size, module)
        arguments
            host
            port
            chunk       = 4096
    ...
    function open(self)
        self.module('open', self.host, self.port, self.chunk, self.mode, self.divisor, self.profile, self.level, self.bitrate, self.options, self.buffer_size);
    end
