...
classdef sink_microphone < matlab.System
    properties
    ...
    properties (Nontunable)
        host        = '192.168.1.7'
        port        = hl2ss.stream_port.MICROPHONE
        chunk       = 4096
        profile     = hl2ss.audio_profile.RAW
    ...
    methods (Access = protected)
        function [audio_size, audio_type] = getAudioType(obj)
            if (obj.profile == hl2ss.audio_profile.RAW)
                if (obj.level == hl2ss.aac_level.L5)
                    audio_size = [hl2ss.parameters_microphone.ARRAY_CHANNELS, hl2ss.parameters_microphone.GROUP_SIZE_RAW];
                    audio_type = 'single';
                else
                    audio_size = [hl2ss.parameters_microphone.CHANNELS, hl2ss.parameters_microphone.GROUP_SIZE_RAW];
                    audio_type = 'int16';
                end
            else
        ...
        function setupImpl(obj)
            ...
            obj.client = hl2ss.mt.sink_microphone(obj.host, obj.port, obj.chunk, obj.profile, obj.level, obj.buffer_size, @hl2ss_matlab);
