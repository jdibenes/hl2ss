...
// OK
void Channel_RM_ZHT::Thunk_Encoder(void* encoded_frame, DWORD encoded_size, LONGLONG sample_time, void* metadata, UINT32 metadata_size, void* self)
{
    static_cast<Channel_RM_ZHT*>(self)->OnEncodingComplete(encoded_frame, encoded_size, sample_time, metadata, metadata_size);
}
...
// OK
void Channel_RM_ZHT::OnEncodingComplete(void* encoded, DWORD encoded_size, LONGLONG sample_time, void* metadata, UINT32 metadata_size)
{
    (void)sample_time;
    (void)metadata_size;
...
void Channel_RM_ZHT::Execute_Mode0(bool enable_location)
{
    ...
    ok = ReceiveZABFormat_Profile(m_socket_client, zabformat);
    if (!ok) { return; }
...
// OK
void RM_ZHT_Initialize()
{
    g_channel = std::make_unique<Channel_RM_ZHT>("RM_ZHT", PORT_NAME_RM_ZHT, PORT_NUMBER_RM_ZHT - PORT_NUMBER_BASE);
}
